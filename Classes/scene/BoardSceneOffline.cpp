...
Scene* BoardSceneOffline::createScene() {
    return BoardSceneOffline::create();
}

bool BoardSceneOffline::init() {
    if (!Scene::init()) {
        return false;
    }
    ...
    auto cancel = MenuItemImage::create(
        ...
        CC_CALLBACK_1(BoardSceneOffline::cancelCallback, this)
    );
    ...
    listener->onTouchBegan = CC_CALLBACK_2(BoardSceneOffline::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(BoardSceneOffline::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(BoardSceneOffline::onTouchEnded, this);
    _eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
    // 创建鼠标移动监听器
    auto mouseListener = EventListenerMouse::create();
    mouseListener->onMouseMove = CC_CALLBACK_1(BoardSceneOffline::onMouseMove, this);
...
//void BoardSceneOffline::initEnemyCards() {
//    // 初始化4张敌方随从卡牌
//    for (int i = 0; i < 4; i++) {
...
// 添加卡牌的属性标签
void BoardSceneOffline::addCardStats(Sprite* card, int health, int attack, int cost, string type) {
    // 存储卡牌属性
...
// 更新卡牌上的属性标签
void BoardSceneOffline::updateCardStats(Sprite* card)
{
    if (!card) return;
...
// 创建出牌区域
void BoardSceneOffline::createDropArea() {
...
// 检查卡牌是否在出牌区域
void BoardSceneOffline::checkDropArea() {
    if (!selectedCard)
        return;
...
// 瞄准 选择攻击对象
void BoardSceneOffline::createAttackIndicator(const Vec2& startPos) {
    if (attackIndicator) {
...
// 攻击动画（通过索引找）（敌我公用）
void BoardSceneOffline::attackmove(int attackerIndex, int defenderIndex) {
    if (isPlayer1Turn)
    {
...
void BoardSceneOffline::spellmove(Sprite* Target) {
    // 创建法术图标精灵
...
// 移除卡牌并播放动画
void BoardSceneOffline::removeCardWithAnimation(Sprite* card) {
    if (!card) return;
    // 先从 cardStats 中移除
...
// 更新敌人的随从位置
void BoardSceneOffline::updateEnemyCardsPosition() {
    Size visibleSize = Director::getInstance()->getVisibleSize();
...
// 将卡牌放回手牌
void BoardSceneOffline::returnCardToHand(Sprite* card) {
...
// 鼠标移动处理
void BoardSceneOffline::onMouseMove(Event* event) {
...
// 缩放Sprite
void BoardSceneOffline::scaleSprite(Sprite* sprite, float scale) {
    sprite->runAction(ScaleTo::create(0.1f, scale));
}

// 出牌
void BoardSceneOffline::removeCard(Sprite* sprite) {
    if (!sprite) return;
...
// 更新己方随从的位置
void BoardSceneOffline::updatePlayedCardsPosition() {
    Size visibleSize = Director::getInstance()->getVisibleSize();
...
// 初始化玩家信息
void BoardSceneOffline::initPlayers() {
...
// 创建玩家UI
void BoardSceneOffline::createPlayerUI() {
...
// 更新玩家UI
void BoardSceneOffline::updatePlayerUI() {
    // 更新生命值显示
...
// 初始化牌堆
void BoardSceneOffline::initDecks() {
    auto visibleSize = Director::getInstance()->getVisibleSize();
    // 创建牌堆容器节点
...
// 从牌堆中抽一张牌
void BoardSceneOffline::drawCard() {
...
// 添加游戏结束检查
void BoardSceneOffline::checkGameOver() {
